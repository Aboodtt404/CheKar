...
def train(data_yaml: str, weights: str, experiment_name: str, epochs: int = 300, imgsz: int = 1024) -> Path:
    ...
    results = model.train(
        ...
        dropout=0.1,
        mixup=0.1,
        copy_paste=0.0,       # disabled — mixed bbox/polygon labels crash seg pipeline
        multi_scale=False,    # disabled — causes ZeroDivisionError with small batch + seg
        overlap_mask=True,    # allow overlapping masks (dent + scratch on same area)
